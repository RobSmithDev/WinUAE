...
#define GW_MFM_BUFFER_MAX_TRACK_LENGTH			0x3800

// Max number of cylinders we can offer
#define GW_MAX_CYLINDER_BRIDGE 82

class GreaseWeazleDiskBridge : public FloppyDiskBridge {
private:
	...
	struct TrackToWrite {
		...
		unsigned char mfmBuffer[GW_MFM_BUFFER_MAX_TRACK_LENGTH];

		// Which side to write it to
		GreaseWeazle::DiskSurface side;
	...
	struct MFMCache {
		...
		GreaseWeazle::GWMfmSample mfmBuffer[GW_MFM_BUFFER_MAX_TRACK_LENGTH];
	...
	MFMCaches m_mfmRead[GW_MAX_CYLINDER_BRIDGE][2];

	// The main thread
	std::thread* m_control;

	bool m_useDriveA;				// Use Drive A or B
	...
	GreaseWeazle::DiskSurface m_floppySide;				// What WinUAE thinks
	GreaseWeazle::DiskSurface m_actualFloppySide;		// Where we currently are
	...
	void internalSwitchCylinder(int cylinder, GreaseWeazle::DiskSurface side);

	// Save a new disk side and switch it in if it can be
	void saveNextBuffer(int cylinder, GreaseWeazle::DiskSurface side);

	// Reset and clear out any data we have received thus far
	void resetWriteBuffer();

public:
	GreaseWeazleDiskBridge(const int device_settings);
	...
	virtual unsigned char getMaxCylinder() override { return GW_MAX_CYLINDER_BRIDGE; };
